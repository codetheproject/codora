...
impl<A: BotApi> Bot<A> {
    ...
    where
        F: FnOnce(A::Option) -> A::Option,
    {
        let bot_api_option = func(Default::default());
        let bot_api = A::from(bot_api_option);
        Bot { bot_api }
    }
}
...
#[cfg(test)]
mod tests {
    use crate::{Bot, LongPoll, Telegram, handler, telegram_types::Updates};
    use anyhow::Result;
    ...
    async fn test_bot_with_long_polling_or_webhook() -> anyhow::Result<()> {
        ...
        let bot: Bot<Telegram> = Bot::new(|option| option).on("/start", handler(start_handler))?;
